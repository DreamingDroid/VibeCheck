...
export async function getWebUserHandler(req: Request, res: Response, pool: Pool) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT email, name, categories, phone_number FROM web_users WHERE email = $1`,
      [email]
    );
    if (rows.length === 0) {
      return res.json({ success: true, data: { email, categories: [], phone_number: null } });
    }
...
export async function saveWebUserHandler(req: Request, res: Response, pool: Pool) {
  const { email, name, categories, phone_number } = req.body;
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO web_users (email, name, categories, phone_number)
      ...
      [email, name || null, JSON.stringify(categories), phone_number || null]
    );

    // If phone number given, also upsert into users (Tier 2 — WhatsApp table)
    // Categories stored under the same JSONB key so WhatsApp handlers can read them
    if (phone_number) {
      await pool.query(
        `INSERT INTO users (phone_number, name, preferences)
         ...
         SET name = EXCLUDED.name,
             ...
             updated_at = CURRENT_TIMESTAMP`,
        [phone_number, name || null, JSON.stringify(categories)]
      );
    }
